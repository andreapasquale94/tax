...
void row( const std::string& name, double ns ) { std::printf( "%-26s %12.2f ns/op\n", name.c_str(), ns ); }
...
void uni()
{
    ...
    row( "mul" + t, bench( it, [&]( std::size_t i ) { E a = mk( jitter( i ) ), b = mk( jitter( i ) + 0.01 ); sink( a * b ); } ) );
    row( "div" + t, bench( it, [&]( std::size_t i ) { E a = mk( jitter( i ) ), b = mk( jitter( i ) + 0.01 ); sink( a / ( b + 2.0 ) ); } ) );
    row( "exp" + t, bench( it, [&]( std::size_t i ) { E a = mk( jitter( i ) ); sink( exp( a ) ); } ) );
    row( "composite" + t, bench( it, [&]( std::size_t i ) { E x = mk( jitter( i ) ); sink( sin( x ) * exp( x ) + sqrt( x * x + 1.0 ) ); } ) );
}
...
void multi( std::size_t it )
{
    ...
    row( "mul" + t, bench( it, [&]( std::size_t i ) { E a = mk( jitter( i ), 0 ), b = mk( jitter( i ), 1 ); sink( a * b ); } ) );
    row( "exp" + t, bench( it, [&]( std::size_t i ) { E a = mk( jitter( i ), 0 ); sink( exp( a ) ); } ) );
    row( "composite" + t, bench( it, [&]( std::size_t i ) { E x = mk( jitter( i ), 0 ), y = mk( jitter( i ), 1 ); sink( sin( x ) * exp( y ) + sqrt( x * x + 1.0 ) ); } ) );
}
}  // namespace
