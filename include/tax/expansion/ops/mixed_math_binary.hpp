...
// Binary math surface for MixedExpansion: pow, atan2. Mirrors
...
[[nodiscard]] MixedExpansion< T, A... > pow( const MixedExpansion< T, A... >& x, int n ) noexcept
{
    using tax::pow;
    return MixedExpansion< T, A... >{ pow( x.inner(), n ) };
}

/// `x^p` for a real exponent (axis set preserved; requires x.value() > 0).
template < typename T, typename... A >
[[nodiscard]] MixedExpansion< T, A... > pow( const MixedExpansion< T, A... >& x,
                                             std::type_identity_t< T > p ) noexcept
{
    using tax::pow;
    return MixedExpansion< T, A... >{ pow( x.inner(), p ) };
}

/// `x^p` for a Taylor-valued exponent over the union of the operands' axes.
template < typename T, typename... A, typename... B >
[[nodiscard]] auto pow( const MixedExpansion< T, A... >& x,
                        const MixedExpansion< T, B... >& p ) noexcept
{
    using R =
...
template < typename T, typename... A >
[[nodiscard]] MixedExpansion< T, A... > pow( std::type_identity_t< T > s,
                                             const MixedExpansion< T, A... >& x ) noexcept
{
    using tax::pow;
    return MixedExpansion< T, A... >{ pow( s, x.inner() ) };
}

/// `atan2(y, x)` over the union of the two operands' axis sets.
template < typename T, typename... A, typename... B >
[[nodiscard]] auto atan2( const MixedExpansion< T, A... >& y,
                          const MixedExpansion< T, B... >& x ) noexcept
{
    using R =
        detail::MergedMixedTaylorExpansion< T, detail::TypeList< A... >, detail::TypeList< B... > >;
    using tax::atan2;
...
template < typename T, typename... A >
[[nodiscard]] MixedExpansion< T, A... > atan2( const MixedExpansion< T, A... >& y,
                                               std::type_identity_t< T > x ) noexcept
{
    using tax::atan2;
    return MixedExpansion< T, A... >{ atan2( y.inner(), x ) };
}

/// `atan2(y, x)` with a constant `y` (axis set unchanged).
template < typename T, typename... A >
[[nodiscard]] MixedExpansion< T, A... > atan2( std::type_identity_t< T > y,
                                               const MixedExpansion< T, A... >& x ) noexcept
{
    using tax::atan2;
    return MixedExpansion< T, A... >{ atan2( y, x.inner() ) };
}
