...
struct BasisTraits< Legendre >
{
        ...
        const detail::CoeffArray< T, N, M >& b ) noexcept
    {
        detail::convertMultiply< T, N, M >(
            out, a, b, detail::legendreToMonomial< T, N, M >,
            detail::monomialToLegendre< T, N, M > );
    }
        ...
        const detail::CoeffArray< T, N, M >& a ) noexcept
    {
        detail::convertReciprocal< T, N, M >(
            out, a, detail::legendreToMonomial< T, N, M >,
            detail::monomialToLegendre< T, N, M > );
    }

    /// @brief Evaluate univariate Legendre polynomial using Clenshaw algorithm.
    /// @details Recurrence: P_{k+1} = ((2k+1)/(k+1))*x*P_k - (k/(k+1))*P_{k-1}
        ...
        else
        {
            return detail::evaluateMultivariate< T, N, M >(
                c, x, []( std::array< T, N + 1 >& v, T xi ) constexpr noexcept {
                    ...
                    for ( int k = 2; k <= N; ++k )
                        v[k] = ( T( 2 * k - 1 ) * xi * v[k - 1] - T( k - 1 ) * v[k - 2] )
                               / T( k );
                } );
        }
    }
        ...
        const detail::CoeffArray< T, N, M >& in, int var ) noexcept
    {
        detail::convertDifferentiate< T, N, M >(
            out, in, var, detail::legendreToMonomial< T, N, M >,
            detail::monomialToLegendre< T, N, M > );
    }
