 ...
 */
template < typename E >
using stored_t = std::conditional_t< std::is_base_of_v< tax::ExpansionLeaf, std::remove_cvref_t< E > >,
                                     const std::remove_cvref_t< E >&, std::remove_cvref_t< E > >;

template < typename E >
/// @brief `true` when `E` is a materialized DA leaf type.
static constexpr bool is_leaf_v = std::is_base_of_v< tax::ExpansionLeaf, std::remove_cvref_t< E > >;
