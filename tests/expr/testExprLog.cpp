...
TEST( Log10, MatchesLogScaled )
{
    ...
    TE< 5 > r1 = log10( x );
    TE< 5 > r2 = log( x ) * ( 1.0 / std::log( 10.0 ) );
    ExpectCoeffsNear( r1, r2 );
}
...
{
    auto [x, y] = TEn< 3, 2 >::variables( { 10.0, 2.0 } );
    TEn< 3, 2 > r1 = log10( x * y );
    TEn< 3, 2 > r2 = log10( x ) + log10( y );
    ExpectCoeffsNear( r1, r2 );
}

TEST( Log10, OfExpression )
{
    auto x = TE< 4 >::variable< 0 >( { 5.0 } );
    TE< 4 > r1 = log10( x * 2.0 );
    TE< 4 > r2 = log( x * 2.0 ) * ( 1.0 / std::log( 10.0 ) );
    ExpectCoeffsNear( r1, r2 );
}
