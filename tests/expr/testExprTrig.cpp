...
{
    auto [x, y] = TEn< 4, 2 >::variables( { 0.7, -1.3 } );
    TEn< 4, 2 > s = sin( x + y );
    TEn< 4, 2 > c = cos( x + y );
...
TEST( Tan, Bivariate )
{
    ...
    TEn< 3, 2 > r1 = tan( x + y );
    TEn< 3, 2 > r2 = sin( x + y ) / cos( x + y );
    ExpectCoeffsNear( r1, r2 );
}

TEST( Tan, OfExpression )
{
    auto x = TE< 4 >::variable< 0 >( { 0.3 } );
    TE< 4 > r1 = tan( x * 2.0 );
    TE< 4 > r2 = sin( x * 2.0 ) / cos( x * 2.0 );
    ExpectCoeffsNear( r1, r2 );
}
