...
TEST( AddTo, MulLeaves_Plus_MulLeaves )
{
    ...
    TE< 4 > r = x * two + x * three;  // x*2 + x*3 = 5x = 5 + 5δ
...
TEST( AddTo, SumExpr_FiveOperands )
{
    ...
    TE< 2 > a{ 1.0 }, b{ 2.0 }, c{ 3.0 }, d{ 4.0 }, e{ 5.0 };
    TE< 2 > r = a + b + c + d + e;
...
{
    auto x = TE< 4 >::variable< 0 >( { 1.0 } );
    TE< 4 > two{ 2.0 }, one{ 1.0 };
    TE< 4 > r = x * two - x * one;  // x*(2-1) = x = 1+δ
...
TEST( SubTo, MixedAddSubChain )
{
    ...
    TE< 4 > one{ 1.0 }, two{ 2.0 }, three{ 3.0 };
    TE< 4 > r = x + one - two + three;  // x + 2
