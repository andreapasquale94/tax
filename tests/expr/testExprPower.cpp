...
TEST( Sqrt, SqrtSquaredIsIdentity )
{
    ...
    auto x = TE< 5 >::variable< 0 >( { 4.0 } );
    TE< 5 > r = square( sqrt( x ) );
...
TEST( Sqrt, Bivariate )
{
    auto [x, y] = TEn< 3, 2 >::variables( { 4.0, 1.0 } );
    TEn< 3, 2 > r1 = sqrt( x );
...
TEST( Cbrt, Bivariate )
{
    auto [x, y] = TEn< 3, 2 >::variables( { 8.0, 1.0 } );
    TEn< 3, 2 > r1 = cbrt( x );
