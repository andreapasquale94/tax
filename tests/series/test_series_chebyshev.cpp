...
{
    auto x = ChebyshevExpansion< 5 >::variable();
...
{
    // x * x = (T_0 + T_2)/2 == x^2.
    auto x = ChebyshevExpansion< 4 >::variable();
    auto sq = x * x;
...
TEST( SeriesChebyshev, DerivativeClosedForms )
{
    // d/dx T_1 = 1 = T_0
    {
        auto f = ChebyshevExpansion< 3 >::variable();  // T_1
        auto d = f.deriv();
