...
TEST( Aliasing, MinusEqSelf_IsZero )
{
    ...
    DA< 4 > r = x;
    r -= r;
    for ( std::size_t k = 0; k < DA< 4 >::ncoef; ++k ) EXPECT_NEAR( r[k], 0.0, kTol );
}
...
TEST( Compose, SumOfProducts_AllLeaves )
{
    ...
    auto [x, y, z] = DAn< 2, 3 >::variables( { 0.0, 0.0, 0.0 } );
    DAn< 2, 3 > r = x * x + y * y + z * z;
...
TEST( Compose, NegatedProductInSum )
{
    ...
    auto [x, y, z] = DAn< 2, 3 >::variables( { 1.0, 1.0, 1.0 } );
    DAn< 2, 3 > r = x * y - z * z;
...
TEST( Compose, NestedSquareAndCube )
{
    ...
    auto x = DA< 6 >::variable< 0 >( { 1.0 } );
    DA< 6 > r1 = square( cube( x ) );
    DA< 6 > r2 = x * x * x * x * x * x;
    ExpectCoeffsNear( r1, r2 );
}
...
{
    // sqrt(x*y) at (4,9) = 6
    auto [x, y] = DAn< 2, 2 >::variables( { 4.0, 9.0 } );
    DAn< 2, 2 > r = sqrt( x * y );
