...
| `tax::la::mean(F)` / `covariance(F)` / `skewness(F)` / `excessKurtosis(F)` / `skewnessTensor(F)` / `kurtosisTensor(F)` | statistical moments of a polynomial map (per-component coefficients + full joint tensors), `x ~ N(0, I)` |
